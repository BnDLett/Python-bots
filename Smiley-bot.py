...
config.read('cryofluidisglorifiedwater.properties')
print(config.get("load", "loadedconfig"))
class MyClient(discord.Client):
    async def on_ready(self):
        print('Logged on as {0}!'.format(self.user))

bot = commands.Bot(command_prefix=config.get("settings", "prefix")+" ")
...
@bot.event
async def on_ready():
    print(config.get("on-ready", "login"))
    print(bot.user.name)
    print(bot.user.id)
    print(bot.latency)
    print(config.get("on-ready", "ready"))
    print(config.get("on-ready", "divider"))
...
async def new_file(ctx, filename: str, *, data):
    ...
    newfile.write("Made via commands: \n"+str(data))
    newfile.close()
